...
function getSessionEvaluate(session) {
...
async function performScroll(session, direction, amount, selector) {
    ...
    return evaluate((input) => {
        ...
        const findBestScrollableContainer = () => {
        ...
        const scrollElement = (element, targetSelector) => {
        ...
        const explicitTarget = input.selector
            ...
            : null;
        if (explicitTarget) {
            return scrollElement(explicitTarget, toSelector(explicitTarget));
        }
        ...
        if (fallbackTarget) {
            return scrollElement(fallbackTarget, toSelector(fallbackTarget));
        }
        return pageResult;
    }, { amount, direction, selector: selector ?? null });
}
export function scrollTool(client) {
    return {
        name: "steel_scroll",
        label: "Scroll",
        ...
        async execute(_toolCallId, params, signal, onUpdate, _ctx) {
            return withToolError("steel_scroll", async () => {
                ...
                const result = await withAbortSignal(performScroll(session, direction, amount, selector), signal);
