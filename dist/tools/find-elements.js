...
function sessionDetails(session, url) {
    return {
        ...baseSessionDetails(session),
        url,
    };
}
async function discoverElements(session, input) {
    ...
    const results = await evaluate((params) => {
        ...
        const candidates = source
            ...
            const searchBlob = toLower(`${text} ${element.getAttribute("aria-label") || ""} ${element.getAttribute("title") || ""}`);
...
export function findElementsTool(client) {
    return {
        name: "steel_find_elements",
        label: "Find Elements",
        description: "Discover likely interactive elements and selector candidates",
        parameters: Type.Object({
            query: Type.Optional(Type.String({ description: "Optional text query to filter by visible label/text" })),
            tag: Type.Optional(Type.String({ description: "Optional exact tag name filter (e.g. button, a, input)" })),
            role: Type.Optional(Type.String({ description: "Optional exact ARIA role filter (e.g. button, link)" })),
            limit: Type.Optional(Type.Integer({
                minimum: 1,
                maximum: MAX_RESULT_LIMIT,
                description: "Max number of candidates to return",
            })),
            clickableOnly: Type.Optional(Type.Boolean({ description: "When true, include only likely interactive elements" })),
        }),
        async execute(_toolCallId, params, signal, onUpdate, _ctx) {
            return withToolError("steel_find_elements", async () => {
                ...
                const candidates = await withAbortSignal(discoverElements(session, {
                    query,
                    tag,
                    role,
                    limit,
                ...
                await emitProgress(onUpdate, "steel_find_elements", `Found ${candidates.length} candidate element(s)`);
                return {
...
//# sourceMappingURL=find-elements.js.map