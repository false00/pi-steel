...
import { emitProgress, sleepWithSignal, throwIfAborted, withAbortSignal, withToolError, } from "./tool-runtime.js";
import { MAX_TOOL_TIMEOUT_MS, MIN_TOOL_TIMEOUT_MS, resolveToolTimeoutMs, } from "./tool-settings.js";
...
function getWaitFunction(session) {
    if (typeof session.waitForSelector === "function") {
        return async (selector, state, timeoutMs, signal) => {
            throwIfAborted(signal);
            await withAbortSignal(session.waitForSelector?.(selector, { state, timeout: timeoutMs }), signal);
        };
    }
    if (typeof session.page?.waitForSelector === "function") {
        return async (selector, state, timeoutMs, signal) => {
            throwIfAborted(signal);
            await withAbortSignal(session.page?.waitForSelector?.(selector, { state, timeout: timeoutMs }), signal);
        };
    }
    ...
    return async (selector, state, timeoutMs, signal) => {
        ...
        while (true) {
            throwIfAborted(signal);
            const isMatched = await withAbortSignal(evaluate((input) => {
...
export function waitTool(client) {
    return {
        name: "steel_wait",
        label: "Wait",
        ...
        parameters: Type.Object({
            ...
            state: Type.Optional(Type.Union([Type.Literal("attached"), Type.Literal("visible")], {
